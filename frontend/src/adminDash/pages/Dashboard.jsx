...
export default function Dashboard() {
  ...
  return (
    <div className="relative space-y-4 sm:space-y-6 px-3 sm:px-4 w-full max-w-[100svw] overflow-x-clip">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-lg sm:text-xl font-semibold" style={{ color: themeDark }}>
          {isAdmin ? "Admin overview" : isClient ? "Client overview" : isFreelancer ? "Freelancer overview" : "Dashboard"}
        </h1>
        <div className="hidden sm:flex gap-2">
          {quickActions.map((q) => (
            <Quick key={q.label} to={q.to} icon={q.icon} label={q.label} />
          ))}
        </div>
      </div>

      {/* KPIs */}
      <div className={`grid gap-3 sm:gap-4 ${isAdmin ? "grid-cols-2 md:grid-cols-3 xl:grid-cols-6" : "grid-cols-2 md:grid-cols-3 xl:grid-cols-5"}`}>
        {kpiList.map((k) => (
      ...
      {isAdmin ? (
        <div className="grid gap-3 sm:gap-4 xl:grid-cols-[minmax(560px,2fr)_minmax(340px,1fr)]">
          {/* Revenue chart */}
          <div className="rounded-2xl border border-slate-200 bg-white/80 backdrop-blur p-4 sm:p-5 shadow-sm">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white/80 backdrop-blur p-4 sm:p-5 shadow-sm min-w-0">
            <div className="mb-3 font-semibold text-slate-800">Pending verifications</div>
            ...
            <ul className="space-y-3">
              {verifs.map((v) => {
                const id = v.id ?? v._id;
                return (
                  <li key={id} className="rounded-xl border border-slate-200 bg-white p-3 sm:p-4 overflow-hidden">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">
                      ...
                      <div className="flex gap-2 sm:justify-end flex-wrap no-wrap-1700">
                        <GradientButton onClick={() => approve(id)} className="flex-1 sm:flex-none gap-1" title="Approve">
                          <CheckCircle /><span className="text-sm">Approve</span>
                        </GradientButton>
                        <OutlineButton onClick={() => reject(id)} title="Reject">
                          <XCircle /><span className="text-sm">Reject</span>
                        </OutlineButton>
                      </div>
                    </div>
                  </li>
        ...
        <div className="grid gap-3 sm:gap-4 lg:grid-cols-2">
          <Card
            ...
            action={
              <Link className="text-sm text-slate-600 hover:underline"
          ...
          >
            <ul className="sm:hidden space-y-3">
              {projects.length === 0 && (
                <li className="rounded-xl border border-slate-200 bg-white p-3 text-slate-500 text-center">No data.</li>
              )}
              {projects.map((p) => (
                <li key={p.id ?? p._id} className="rounded-xl border border-slate-200 bg-white p-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="text-slate-800 font-medium">{p.title}</div>
                      <div className="text-xs text-slate-500">{p.client} • {p.status}</div>
                    </div>
        ...
        <div className="grid gap-3 sm:gap-4 lg:grid-cols-2">
          <Card title="Recent signups">
            <ul className="sm:hidden space-y-3">
              ...
              {signups.map((s) => (
                <li key={s.id ?? s._id} className="rounded-xl border border-slate-200 bg-white p-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <div className="text-slate-800 font-medium">{s.name}</div>
                      <div className="text-xs text-slate-500">{s.type}{s.city ? ` • ${s.city}` : ""}</div>
                    </div>
          ...
          <Card
            ...
            action={<Link className="text-sm text-slate-600 hover:underline" to="/admin/operation/projects">View all</Link>}
          >
            <ul className="sm:hidden space-y-3">
              {projects.length === 0 && (
                <li className="rounded-xl border border-slate-200 bg-white p-3 text-slate-500 text-center">No data.</li>
...
function Kpi({ title, value, icon }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white/80 backdrop-blur p-3 sm:p-4 shadow-sm">
            ...
            <tbody className="divide-y divide-slate-200 text-slate-800 text-sm">
              ...
              {rows.map((cells, i) => (
                <tr key={i} className="hover:bg-slate-50">
                  {cells.map((c, j) => (
                    <td key={j} className="px-3 sm:px-4 py-3 truncate">{c}</td>
                  ))}
                </tr>
              ))}
            </tbody>
