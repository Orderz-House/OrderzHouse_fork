...
import { ChevronDown, Menu, X, Search, Bell, User, Settings, LogOut, Globe, Phone, Mail, MapPin } from 'lucide-react';
...
export default function EnhancedNavbar() {
  ...
  const contactInfo = [
    ...
    { name: 'Visit Us', desc: '123 Business Ave, NY', icon: MapPin },
    { name: 'Global Offices', desc: 'Worldwide locations', icon: Globe }
  ];

  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      isScrolled ? 'bg-white/95 backdrop-blur-md shadow-lg border-b border-gray-200/50' : 'bg-white shadow-sm border-b border-gray-200'
    }`}>
          ...
          <div className="hidden lg:block">
            <div className="flex items-center space-x-1">
              ...
              <div className="relative" ref={contactRef}>
                ...
                >
                  CONTACT
                  <ChevronDown className={`ml-1 h-4 w-4 transition-transform duration-200 ${isContactOpen ? 'rotate-180' : ''}`} />
                </button>
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 border-t border-gray-200 bg-white/95 backdrop-blur-md">
              {['HOME', 'ABOUT US', 'SERVICES', 'CONTACT', 'BLOGS'].map((link) => (
                <button
                  key={link}
                  onClick={() => setActiveLink(link)}
