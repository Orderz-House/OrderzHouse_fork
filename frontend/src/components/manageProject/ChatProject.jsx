...
import { MessageSquare, Send, User, AtSign, X, ImageIcon, Paperclip, Smile } from "lucide-react";
...
function ChatProject() {
  ...
  const formatMessage = (text) => {
    ...
    return text.replace(/@\[([^\]]+)\]\(([^)]+)\)/g, (match, name, id) => {
      ...
      >
        {messages.length === 0 ? (
          ...
          messages.map((message, index) => {
            ...
            const showHeader = !isCurrentUser && (
              index === 0 || 
              messages[index - 1].sender_id !== message.sender_id ||
              new Date(message.time_sent).getTime() - new Date(messages[index - 1].time_sent).getTime() > 300000 // 5 minutes
            );
                ...
                <div className={`max-w-xs lg:max-w-md ${isCurrentUser ? "ml-12" : "mr-12"}`}>
                  ...
                  <div className="flex items-start">
                    {!isCurrentUser && !showAvatar && (
                      <div className="w-6 h-6 flex-shrink-0 mr-2 flex justify-center">
                    ...
                    {!isCurrentUser && showAvatar && (
                      <div className="flex-shrink-0 mr-2">
                        {message.sender.sender_avatar ? (
                          ...
                          <div className={`w-6 h-6 rounded-full ${getAvatarColor(message.sender_id)} text-white flex items-center justify-center text-xs font-medium`}>
                            
                            {getUserInitials(message.sender.first_name)}
                          </div>
                        )}
                      </div>
                    )}
              ...
              filteredMembers.map((member) => (
                ...
                >
                  {member.avatar ? (
                    ...
                    <div className={`w-8 h-8 rounded-full ${getAvatarColor(member.id)} text-white flex items-center justify-center text-sm font-medium mr-3`}>
