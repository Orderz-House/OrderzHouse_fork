...
import { fetchCategories } from "./api/category"; // Import API function

/* ============== CategoriesRail (chips only) ============== */
export function CategoriesRail({ active, onSelect, theme = "#028090", themeDark = "#05668D" }) {
  ...
  if (loading) {
    return (
      <div className="sticky top-0 z-30 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 mb-5">
        <div className="relative rounded-2xl bg-white/70 backdrop-blur shadow-[0_10px_36px_rgba(2,128,144,.12)] ring-1 ring-black/5 p-4">
...
export default function Topbar({
  active, onSelect, theme = "#028090", themeDark = "#05668D", themeLight = "#9AE6B4",
...
function TopbarCategories({ active, onSelect, theme = "#028090", themeDark = "#05668D" }) {
  const [categories, setCategories] = useState([]);
  const [loading, setLoading] = useState(true);
...
/* -------- CategoryBar (updated to fetch from API) ---------- */
function CategoryBar({
  cat, sub, total, onSelectSub, sort, setSort, pro, setPro, instant, setInstant,
  theme = "#028090", themeLight = "#9AE6B4", themeDark = "#05668D", filters, onChangeFilters,
}) {
  ...
  useEffect(() => {
    ...
    const loadCategory = async () => {
      ...
      try {
        ...
        const selectedCategory = categories.find(c => (c._id || c.id) === cat);
        
        if (selectedCategory) {
          setCategoryData({
            title: selectedCategory.name || "Projects",
            subtitle: selectedCategory.description || "Browse projects by category and filters",
            subcats: (selectedCategory.subCategories || []).map(sub => ({
              id: sub._id || sub.id,
              name: sub.name
            }))
          });
      ...
      {loading ? (
        ...
        <>
          <div className="relative z-10 flex items-end justify-between gap-3 flex-wrap">
            <div>
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-black tracking-tight" style={{ color: themeDark }}>{categoryData.title}</h1>
              <p className="mt-1 text-slate-600 text-sm sm:text-base">{categoryData.subtitle}</p>
            </div>
            <div className="text-sm text-slate-500">{total.toLocaleString()} results</div>
          </div>

          {categoryData.subcats && categoryData.subcats.length > 0 && (
