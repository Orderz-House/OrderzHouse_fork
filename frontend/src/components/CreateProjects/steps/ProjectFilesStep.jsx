    ...
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm p-8">
      ...
      <div
        className={`rounded-2xl p-10 text-center border-2 border-dashed transition-all ${
          dragActive ? "border-[#028090] bg-[#E6F7F6]" : "border-slate-300 hover:border-[#028090]/50"
        }`}
      ...
      {files.length > 0 && (
        <div className="mt-6">
          <h3 className="font-semibold text-slate-800 mb-3">Selected Files ({files.length})</h3>
          <ul className="space-y-2">
            {files.map((file, index) => (
              <li key={index} className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-200">
                <div className="flex items-center gap-3">
                  <svg className="w-5 h-5" style={{ color: THEME }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
