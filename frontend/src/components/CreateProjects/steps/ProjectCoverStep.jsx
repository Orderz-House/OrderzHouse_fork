...
export default function ProjectCoverStep({ coverPic, setCoverPic, onNext, onBack }) {
  const [preview, setPreview] = useState(coverPic ? URL.createObjectURL(coverPic) : null);
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm p-8">
      <div className="mb-6">
        <h2 className="text-2xl font-black tracking-tight text-slate-900">Project Cover Image</h2>
        <p className="text-slate-600">Upload a nice preview image for your project (optional)</p>
      </div>
        ...
        {!preview ? (
          <>
            ...
            <label htmlFor="cover-upload" className="cursor-pointer block">
              ...
              <p className="text-slate-600 text-lg">Click to upload cover image</p>
              <p className="text-slate-400 text-sm mt-1">You can skip this step if you prefer</p>
            </label>
          </>
        ) : (
