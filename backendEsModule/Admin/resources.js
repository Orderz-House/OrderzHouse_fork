// Admin/resources.js - Complete resource configurations
...
export const createResourceConfigs = async (
          ...
          after: async (response, request, context) => {
            if (context.currentAdmin && request.payload) {
              await logAdminAction(
                ...
                `Admin created new ${navigationName
                  .toLowerCase()
                  .slice(0, -1)}: ${request.payload.email} (${
          ...
          after: async (response, request, context) => {
            if (context.currentAdmin && request.payload) {
              await logAdminAction(
                context.currentAdmin.id,
                context.currentAdmin.email,
                `Admin updated ${navigationName
                  .toLowerCase()
                  .slice(0, -1)} ID: ${request.params.recordId}`
              );
            }
            return response;
          },
          ...
          after: async (response, request, context) => {
            if (context.currentAdmin) {
              await logAdminAction(
                context.currentAdmin.id,
                context.currentAdmin.email,
                `Admin DELETED ${navigationName
                  .toLowerCase()
                  .slice(0, -1)} with ID: ${request.params.recordId}`
              );
            }
            return response;
          },
  ...
  const resources = [
    ...
    // User Management - Filtered Views
  ...
  // Check for additional tables and add verification resources if they exist
  try {
    // Use a more reliable method to check table existence
    const checkAdditionalTables = async () => {
      const pool = db.connectionDetails.pool || db.pool;
      if (!pool)
        return {
          freelancer_verifications: false,
          customer_verifications: false,
          freelancer_verification_categories: false,
        };
      ...
      try {
        const tableChecks = await Promise.allSettled([
          client.query(
            `SELECT table_name FROM information_schema.tables WHERE table_name = 'freelancer_verifications'`
          ),
          client.query(
            `SELECT table_name FROM information_schema.tables WHERE table_name = 'customer_verifications'`
          ),
          client.query(
            `SELECT table_name FROM information_schema.tables WHERE table_name = 'freelancer_verification_categories'`
          ),
        ]);
        ...
        return {
          freelancer_verifications:
            ...
            tableChecks[0].value.rows.length > 0,
          customer_verifications:
            tableChecks[1].status === "fulfilled" &&
            tableChecks[1].value.rows.length > 0,
          freelancer_verification_categories:
            tableChecks[2].status === "fulfilled" &&
            tableChecks[2].value.rows.length > 0,
        };
      } finally {
        client.release();
      }
    ...
    const additionalTables = await checkAdditionalTables();

    // Freelancer Verifications
    if (additionalTables.freelancer_verifications) {
      resources.push({
        ...
        options: {
          ...
          sort: { sortBy: "reviewed_at", direction: "desc" },
          properties: {
            user_id: {
              reference: "users",
              type: "reference",
    ...
    if (additionalTables.customer_verifications) {
      resources.push({
    ...
    // Freelancer Verification Categories
    if (additionalTables.freelancer_verification_categories) {
      resources.push({
    ...
    console.error(
      "Error checking additional verification tables:",
      error.message
    );
  }
  ...
  console.log(
    `🔗 Resource IDs: ${resources.map((r) => r.options.id).join(", ")}`
  );
