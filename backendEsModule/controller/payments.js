...
/**
 * Freelancer takes a project - first come, first served.
 * When a freelancer takes it:
 *  - project.assigned_freelancer_id is set
 *  - project.status = 'in_progress'
 *  - create escrow record (escrow.freelancer_id NOT NULL required by schema)
 *    escrow amount comes from the approved payment (payments.order_id = payment id)
 */
export const takeProject = async (req, res) => {
  ...
  try {
    ...
    const projectRes = await client.query(
      `SELECT id, user_id, budget, status, assigned_freelancer_id
       FROM projects
       WHERE id = $1
       FOR UPDATE`,
      [projectId]
    ...
    if (project.assigned_freelancer_id) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Project already taken" });
    }
    ...
    if (project.status !== "available") {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Project not available for taking" });
    }

    // Find approved payment for this project
    const paymentRes = await client.query(
      `SELECT id, payer_id, amount
       FROM payments
       WHERE project_id = $1 AND order_id = id -- our "approved" marker
       FOR UPDATE`,
      [projectId]
    );
    ...
    if (!paymentRes.rows.length) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "No approved payment found for this project",
        });
    }
    ...
    // assign project
    await client.query(
      `UPDATE projects SET assigned_freelancer_id = $1, status = 'in_progress' WHERE id = $2`,
      [freelancerId, projectId]
    );

    // create escrow (escrow.freelancer_id NOT NULL in schema)
    await client.query(
      `INSERT INTO escrow (project_id, client_id, freelancer_id, amount, status, created_at)
       VALUES ($1, $2, $3, $4, 'held', NOW())`,
      [projectId, payment.payer_id, freelancerId, payment.amount]
    );

    // Optional: insert into completion_history that project assigned/taken
    await client.query(
      `INSERT INTO completion_history (project_id, event, timestamp, actor, notes)
       VALUES ($1, 'completion_requested'::varchar, NOW(), $2, $3)`,
      [
        projectId,
        freelancerId,
        "Freelancer took the project and escrow created",
      ]
    );
    ...
    return res.json({
      success: true,
      message: "Project assigned to you and escrow created",
    });
  } catch (err) {
    await client.query("ROLLBACK");
    console.error("takeProject error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  } finally {
    client.release();
  }
