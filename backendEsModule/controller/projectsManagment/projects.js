 ...
 * -------------------------------
 * Returns only verified, not-deleted freelancers in this category
 * who are NOT assigned to ongoing projects.
 * -------------------------------
 */
export const getRelatedFreelancers = async (req, res) => {
  ...
  try {
    const { rows: freelancers } = await pool.query(
      `
      SELECT DISTINCT 
        u.id, u.username, u.email, u.country, u.profile_pic_url, u.created_at,
      ...
      WHERE 
        ...
        AND NOT EXISTS (
          ...
          WHERE 
            ...
            AND pa.status IN ('accepted', 'active')
...
export const addProjectFiles = async (req, res) => {
  ...
  try {
    const uploadedFiles = [];
    const uploaded = await uploadFilesToCloudinary(req.files, `projects/${projectId}`);
    ...
    for (const file of uploaded) {
      const { rows } = await pool.query(
        `INSERT INTO project_files 
         ...
         VALUES ($1, $2, $3, $4, $5, $6) 
         RETURNING *`,
        [projectId, userId, file.name, file.url, file.size, file.public_id]
      );
  ...
  try {
    ...
    const { rows: assignmentRows } = await pool.query(
      `INSERT INTO project_assignments 
       ...
       VALUES ($1, $2, NOW(), 'pending_acceptance', 'by_client')
       RETURNING *`,
      [projectId, freelancer_id]
    );
    ...
    await LogCreators.projectOperation(
      ...
      projectId,
      true,
      { freelancer_id, assignment_id: assignment.id, action: "invitation_sent" }
    );
    ...
    return res.status(201).json({
      success: true,
      message: "Invitation sent successfully. Freelancer must accept before assignment becomes active.",
      assignment
    });
  } catch (error) {
